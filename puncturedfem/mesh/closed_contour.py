from numpy import zeros, shape, array, argmax, linspace, meshgrid, inf, sqrt
...
class closed_contour:
	...
	def __init__(self, cell_id: int,
		  ...
		  edge_orients: list[int]=None) -> None:
	...
	def is_in_interior_contour(self, x: array, y: array) -> array:
	...
	### FUNCTION EVALUATION ####################################################
	def evaluate_function_on_contour(self, fun: callable) -> array:
		...
		for j in range(self.num_edges):
			y[self.vert_idx[j]:self.vert_idx[j+1]] \
				= self.edges[j].evaluate_function(fun, ignore_endpoint=True)
		return y
