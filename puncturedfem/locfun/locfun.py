from numpy import ndarray, zeros, empty, pi, nan, log
from . import d2n
from . import antilap
...
class locfun:
	...
	* Weighted normal derivative of anti-Laplacian of harmonic part
	...
	def compute_trace_values(self) -> None:
		...
		for i in range(self.solver.K.num_holes + 1):
			...
			for j in range(c.num_edges):
				...
				self.trace[k_start:k_end] = \
					self.poly_trace.polys[edge_idx].eval(
	...
	def set_polynomial_part_weighted_normal_derivative(self,
						    P_wnd: ndarray) -> None:
	...
	def compute_harmonic_conjugate(self) -> None:
		...
		self.conj_trace, self.log_coef = \
			self.solver.get_harmonic_conjugate(phi_trace)
	...
	### anti-Laplacian #########################################################
	def set_anti_laplacian_harmonic_part(self,
				      anti_laplacian_vals: ndarray) -> None:
	...
	def cauchy_integral_formula(self,
		  i: int, j: int,
		  bdy_x1: ndarray, bdy_x2: ndarray,
		  int_x1: ndarray, int_x2: ndarray,
		  psi: ndarray, psi_hat: ndarray,
		  Px: polynomial, Py: polynomial) -> None:
		...
		# polynomial part
		self.int_vals[i, j] += self.poly_part.eval(
			int_x1[i, j], int_x2[i, j])
		...
		for k in range(self.solver.K.num_holes):
			...
			y_xi_norm_sq = (int_x1[i, j] - xi.x) ** 2 \
				+ (int_x2[i, j] - xi.y) ** 2
			self.int_vals[i, j] += \
				0.5 * self.log_coef[k] * log(y_xi_norm_sq)
